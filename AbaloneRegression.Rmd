...
Dependencies among independent variables is an undesirable qulaity when doing regression analysis and we can observe some strong dependencies especially between *Length* and *Diameter*.  In the the **Results** section, we delve into this in more detail.  There also appears to be some dependencies between the different kinds of weights (Shucked, etc.), but the thickness of the scatter is greater than the thickness between *Length* and *Diameter*.
...
X <- abalone.converted[train.rows,-10]
Y <- abalone.converted[train.rows, 10]
